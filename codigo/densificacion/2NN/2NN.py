...
def densify_point_cloud(points: np.ndarray, remissions: np.ndarray, 
    ...
    for _ in range(num_new_points):
        ...
        neighbor1 = points[indices[0][1]]
        neighbor2 = points[indices[0][2]]

        print(f"\nPunto original seleccionado [{index}]: {point}")
        print(f"Primer vecino encontrado [{indices[0][1]}]: {neighbor1}")
        print(f"Segundo vecino encontrado [{indices[0][2]}]: {neighbor2}")
        
        # Interpolar entre el punto original y el vecino seleccionado aplicando D = A + α(B - A) + β(C - A)
        ...
        interpolated_remission = (remissions[index] + remissions[indices[0][1]] + remissions[indices[0][2]]) / 3
