...
def vis_sequences(path, initial_colormap='plasma', initial_fps=0.5):
    ...
    segmentador = GroundSegmentation(distancia_threshold=0.3, max_angle_deg=3.0, min_inliers=10000)
    ...
    def update_point_cloud():
        ...
        if is_segmented[0]:
            # Si estamos en modo segmentación, aplicar la segmentación
            segmentador.segment_ground(point_cloud)
        else:
            # Si no, usar el colormap normal
            colors = set_colors(remissions[0], current_colormap[0])
            current_colors[0] = colors  # Actualizar los colores actuales
            point_cloud.colors = o3d.utility.Vector3dVector(colors)
