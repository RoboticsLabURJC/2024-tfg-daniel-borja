...
def apply_cosmic_ray(points: np.ndarray, remissions: np.ndarray, labels: np.ndarray, 
                                 impact_probability: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    random_labels = rng.choice(unique_labels, size=num_impacted)
    displaced_labels[impacted_indices] = random_labels

    print("Impacto completado con desplazamientos, remisión y actualización de etiquetas.")
    return displaced_points, displaced_remissions, displaced_labels
...
def batch_cosmic_strike(input_dir: str, output_dir: str, impact_probability: float,
    ...
    for file in tqdm(input_files, desc="Aplicando rayos cósmicos"):
        try:
            ...
            # Leer etiquetas
            labels = None
            # Parte común para buscar archivo label
            ...
            new_points, new_remissions, new_labels = apply_cosmic_ray(points, remissions, labels, impact_probability)
            ...
            new_labels.astype(np.uint32).tofile(label_output_path)
...
def main():
    ...
    while True:
        label_dir = input("📂 Introduzca la ruta del directorio de etiquetas (.label): ").strip()
    ...
    output_dir = os.path.join(input_dir, f"cosmic_output_{impact_prob}")
    os.makedirs(output_dir, exist_ok=True)

    print(f"\n⚙ Configuración:")
    print(f" - Probabilidad de impacto: {impact_prob*100:.1f}%")
    print(f" - Directorio de entrada: {input_dir}")
    print(f" - Directorio de etiquetas: {label_dir}")
    print(f" - Directorio de salida: {output_dir}")

    confirm = input("\n¿Desea continuar? (s/n): ").lower()
    if confirm == 's':
        batch_cosmic_strike(input_dir, output_dir, impact_prob, label_dir)
    else:
        print("\nProceso cancelado.")
