...
from dbscan_segmentation import CloudSegmentation
...
def vis_sequences(path, initial_colormap='plasma', initial_fps=0.5):
    ...
    segmentador = CloudSegmentation(eps=0.5, min_points=10)
    ...
    def toggle_segmentation(vis):
        ...
        if is_segmented[0]:
            # Aplicar segmentación DBSCAN
            segmentador.segment_clusters(point_cloud)
            print("Cambio a vista de segmentación por clusters")
        else:
