...
SAVE_DIR = "/home/dani/2024-tfg-daniel-borja/datasets/CARLA/03"
frame_count = 0
SAVE_INTERVAL = 10 #Guardar cada 10 frames
...
def lidar_callback(lidar_data, point_cloud):
    ...
    # Extraer y procesar los datos del LiDAR
    data = np.copy(np.frombuffer(lidar_data.raw_data, dtype=np.dtype('f4')))
    data = np.reshape(data, (int(data.shape[0] / 4), 4))

    # Invertir eje X (opcional, según el sistema de coordenadas)
    data[:, 0] = -data[:, 0]

    # Crear el PointCloud de Open3D para la visualización
    intensity = data[:, -1]
    int_color = np.c_[
    ...
    # Guardar los datos en un archivo PCD
    if frame_count % SAVE_INTERVAL == 0:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = os.path.join(SAVE_DIR, f"lidar_capture_{timestamp}.pcd")
