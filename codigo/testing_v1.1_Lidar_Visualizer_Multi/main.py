...
def initial_choice():
    ...
    def launch_interface():
        ...
        def start_visualization():
            ...
            paths = path_entry.get().split(";")  # Separar los directorios por ";"
            colormap = colormap_var.get()
            fps = float(fps_var.get())
            
            # Validar los directorios
            for path in paths:
                if not os.path.isdir(path):
                    messagebox.showerror("Error", f"El directorio seleccionado no es válido: {path}")
                    return
            ...
            # Lanzar la visualización con múltiples directorios
            vis_sequences(paths, colormap, fps)
        ...
        ttk.Label(frame, text="Selecciona los Directorios de Datos (separados por ';'):").grid(row=0, column=0, sticky="w")
        path_entry = ttk.Entry(frame, width=40)
        path_entry.grid(row=1, column=0, padx=(0, 10), pady=5)
        ttk.Button(frame, text="Examinar", command=lambda: path_entry.insert(0, ";".join(filedialog.askdirectory(multiple=True))).grid(row=1, column=1, pady=5))
